...
namespace BFBot
    ...
    public class Exchange
    {
        ...
        public string GetAllMarketsUKHorseRacing()
            {
            int?[] ids = new int?[1];
            ids[0] = 7;
            ...
            BetfairE.GetAllMarketsReq marketAll = new BetfairE.GetAllMarketsReq();
            marketAll.header = headerExchange;
            marketAll.eventTypeIds = ids;
            marketAll.countries = country;
            marketAll.fromDate = DateTime.Today;
            marketAll.toDate = DateTime.Today.AddDays(1.0);

            BetfairE.GetAllMarketsResp marketAllResponse = serviceExchangeUk.getAllMarkets(marketAll);
            ResetSessionToken(marketAllResponse.header);

            
            //BFBot.DumpToFile(marketAllResponse.marketData);

            return marketAllResponse.marketData;
            }
